...
function injectStyle() {
  ...
  st.textContent = `
      ...
      /* Tabela ATP: não truncar conteúdo textual das células. */
      #tableAutomatizacaoLocalizadores td,
      #tableAutomatizacaoLocalizadores th{
        text-overflow:clip !important;
        overflow:visible !important;
        max-width:none !important;
      }
      #tableAutomatizacaoLocalizadores [id^="dadosResumidos_"],
      #tableAutomatizacaoLocalizadores [id^="dadosCompletos_"]{
...
function mapColumns(table) {
  ...
  const expectedHeaderScore = (row) => {
    const cells = Array.from(row?.querySelectorAll?.(':scope > th, :scope > td') || []);
    if (!cells.length) return -1;
    const txt = rmAcc(lower(cells.map((c) => clean(c.textContent || '')).join(' | ')));
    let score = 0;
    if (txt.includes('localizador') && (txt.includes('remover') || txt.includes('origem'))) score += 3;
    if (txt.includes('tipo') && (txt.includes('controle') || txt.includes('criterio'))) score += 2;
    if (txt.includes('localizador') && (txt.includes('incluir') || txt.includes('destino') || txt.includes('acao'))) score += 3;
    if (txt.includes('outros') && txt.includes('criterio')) score += 2;
    if (/\bn[º°o]?\b/.test(txt) || txt.includes('n.')) score += 1;
    if (txt.includes('prioridade')) score += 1;
    return score;
  };
  const theadRows = Array.from(thead?.querySelectorAll?.('tr') || []);
  const rows = theadRows.length
    ? theadRows
    : [table?.querySelector?.('tr')].filter(Boolean);
  let headRow = null;
  let bestScore = -1;
  for (const row of rows) {
    const s = expectedHeaderScore(row);
    if (s > bestScore) { bestScore = s; headRow = row; }
  }
  if (!headRow) return null;
  const ths = Array.from(headRow.querySelectorAll(':scope > th, :scope > td'));
  const norm = (txt) => rmAcc(lower(String(txt || '')))
    .replace(/[^\w\s/.-]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  const hasWord = (txt, word) => new RegExp(`\\b${word}\\b`, 'i').test(txt);
  const heads = ths.map((th) => {
    const raw = String(th.textContent || '');
    const txt = norm(raw);
    const tipo = String(th?.dataset?.atpCol || '');
    return { th, raw, txt, tipo };
  });
  const findHeader = (predicate) => heads.findIndex(predicate);
  const findNumero = () => {
    const byDataset = findHeader((h) => h.tipo === 'numero-nucleo');
    if (byDataset >= 0) return byDataset;
    const byLabel = findHeader((h) => {
      if (h.tipo === 'prioridade-nucleo') return false;
      const txt = h.txt;
      if (!txt) return false;
      if (txt.includes('n / prioridade') || txt.includes('n. / prioridade')) return true;
      if (txt === 'n' || txt === 'n.') return true;
      if (/^n[º°o]?\b/.test(txt)) return true;
      return false;
    });
    return byLabel;
  };
  const findRemover = () => findHeader((h) =>
    hasWord(h.txt, 'localizador') && (
      hasWord(h.txt, 'remover') ||
      hasWord(h.txt, 'origem')
    )
  );
  const findTipo = () => {
    const strict = findHeader((h) => {
      const t = h.txt;
      if (!t) return false;
      return (
        t.startsWith('tipo de controle / criterio') ||
        t.startsWith('tipo de controle/criterio') ||
        t.startsWith('tipo de controle e criterio') ||
        t === 'tipo de controle' ||
        t === 'tipo'
      );
    });
    if (strict >= 0) return strict;
    return findHeader((h) => hasWord(h.txt, 'tipo') && (h.txt.includes('controle') || h.txt.includes('criterio')));
  };
  const findIncluir = () => findHeader((h) =>
    hasWord(h.txt, 'localizador') && (
      h.txt.includes('incluir') ||
      h.txt.includes('destino') ||
      h.txt.includes('acao')
    )
  );
  const findOutros = () => {
    ...
    if (strict >= 0) return strict;
    return findHeader((h) => h.txt.includes('outros') && h.txt.includes('criterio'));
  };
  const findPrioridade = () => findHeader((h) => h.tipo === 'prioridade-nucleo' || (h.txt.includes('prioridade') && !h.txt.includes('tipo')));

  const colNumPrior = findNumero();
  const colRemover = findRemover();
  const colTipo = findTipo();
  const colIncluir = findIncluir();
  const colOutros = findOutros();
  const colPrioridade = findPrioridade();

  return {
    colNumPrior: colNumPrior >= 0 ? colNumPrior : 1,
    colPrioridade: colPrioridade >= 0 ? colPrioridade : null,
    colRemover: colRemover >= 0 ? colRemover : 3,
    colTipo: colTipo >= 0 ? colTipo : 4,
    colIncluir: colIncluir >= 0 ? colIncluir : 5,
    colOutros: colOutros >= 0 ? colOutros : 6
  };
}
