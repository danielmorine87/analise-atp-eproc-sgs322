...
function extrairLocalizadorIncluirAcao(tdIncluir) {
  ...
  const limparControlesNaoSemanticos = (root) => {
    try {
      if (!root || !root.querySelectorAll) return;
      root.querySelectorAll(
        'select, option, input, button, textarea, .bootstrap-select, .dropdown-toggle, .dropdown-menu, .btn-group'
      ).forEach((n) => n.remove());
    } catch (_) {}
  };
  ...
  const acoes = (function extrairAcoesProgramadas() {
    try {
      ...
      limparControlesNaoSemanticos(root);
  ...
  limparControlesNaoSemanticos(clone);
...
function normalizarNomeLocalizadorOrigemRemover(raw) {
  return clean(raw || '').replace(/\s+/g, ' ').trim();
}

function removerDecoradoresVisuaisLocalizador(raw) {
  // Remove ícones/decorações semânticamente neutras (ex.: ampulheta/dado).
  return clean(String(raw || '').replace(/[\u231A-\u27BF\u{1F000}-\u{1FAFF}]/gu, ' '));
}

function extrairPrimeiroBlocoTextoRemover(td) {
  if (!td) return '';
  const root = (() => {
    const divComp = td.querySelector && td.querySelector('div[id^="dadosCompletos_"]');
    return (divComp || td).cloneNode(true);
  })();
  try {
    if (!root || !root.querySelectorAll) return '';
    root.querySelectorAll(
  ...
  const txt = removerDecoradoresVisuaisLocalizador(root.innerText || root.textContent || '');
...
function extrairLinhasTextoRemover(td) {
  if (!td) return [];
  const root = (() => {
    const divComp = td.querySelector && td.querySelector('div[id^="dadosCompletos_"]');
    return (divComp || td).cloneNode(true);
...
function quebrarTermoRemoverEmItens(raw) {
  const txt = removerDecoradoresVisuaisLocalizador(raw);
  if (!txt) return [];
  // Quando houver múltiplos itens no mesmo texto, separa apenas antes de novo prefixo "(X)".
  const partes = txt
    .split(/\s*-\s*(?=\([^)]+\)\s*)/g)
    .map((p) => clean(p))
    .filter(Boolean);
  return partes.length ? partes : [txt];
}

function extrairOrigensEstruturaisDoBloco(raw) {
  const txt = removerDecoradoresVisuaisLocalizador(raw);
  if (!txt) return [];
  const blocos = quebrarTermoRemoverEmItens(txt);
  const out = [];
  for (const bloco of blocos) {
    const pedacos = String(bloco)
      .split(/\s+\b(?:E|OU)\b\s+/i)
      .map((p) => clean(p))
      .filter(Boolean);
    let encontrouTrechoNaoLocalizador = false;
    for (const p of pedacos) {
      const temPrefixoEstrutural = /^\([^)]+\)\s*/.test(p);
      if (!temPrefixoEstrutural) {
        encontrouTrechoNaoLocalizador = true;
        break;
      }
      out.push(p);
    }
    if (encontrouTrechoNaoLocalizador) break;
  }
  return out;
}

function termoRemoverEhFiltroNaoOrigem(raw) {
  const src = clean(raw || '');
  if (!src) return true;
  const norm = ((typeof rmAcc === 'function') ? rmAcc(src) : src)
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim();
  if (!norm) return true;
  if (norm === 'e' || norm === 'ou' || norm === '[*]') return true;
  // Filtros/condições textuais que não representam localizador de origem.
  if (/^localizador\s+nao\s+contenha/.test(norm)) return true;
  if (/^localizador\s+que\s+contenha/.test(norm)) return true;
  if (/^peticao\b/.test(norm)) return true;
  if (/^decurso\s+de\s+prazo\b/.test(norm)) return true;
  if (/^tempo\s+no\s+localizador\b/.test(norm)) return true;
  if (/^tempo\s+na\s+situacao\b/.test(norm)) return true;
  if (/^data\b/.test(norm)) return true;
  if (/^periodicamente\b/.test(norm)) return true;
  if (/^tipo\s+de\s+peticao\b/.test(norm)) return true;
  if (/^evento\s+tipo\s+de\s+peticao\b/.test(norm)) return true;
  if (/^classe\b/.test(norm)) return true;
  if (/^assunto\b/.test(norm)) return true;
  if (/^entidade\b/.test(norm)) return true;
  if (/^prazo\b/.test(norm)) return true;
  if (/^ar\s+entregue\b/.test(norm)) return true;
  return false;
}

function extrairListaOrigensRemover(td, exprRemover, firstBlockText, tipoCanonico) {
  const out = new Set();
  const addLinha = (raw) => {
    const nome = normalizarNomeLocalizadorOrigemRemover(raw);
    if (!nome) return;
    const nomeNorm = ((typeof rmAcc === 'function') ? rmAcc(nome) : nome).toLowerCase();
    if (nomeNorm === 'todos os localizadores') return;
    if (termoRemoverEhFiltroNaoOrigem(nome)) return;
    out.add(nome);
  };

  // Modo estável: cada linha do campo ORIGEM vira um item, sem quebrar por E/OU.
  const linhas = extrairLinhasTextoRemover(td);
  for (const ln of linhas) {
    addLinha(ln);
    // Reaproveita parser estrutural para recuperar múltiplas origens no mesmo texto.
    const itensEstruturais = extrairOrigensEstruturaisDoBloco(ln);
    for (const it of itensEstruturais) addLinha(it);
  }

  // Fallback: usa primeira linha textual quando necessário.
  if (!out.size && clean(firstBlockText)) {
    addLinha(firstBlockText);
    const itensEstruturais = extrairOrigensEstruturaisDoBloco(firstBlockText);
    for (const it of itensEstruturais) addLinha(it);
  }

  // Fallback final: varre todos os termos de todas as cláusulas (não apenas o primeiro).
  if (!out.size) {
    const clausesAll = Array.isArray(exprRemover?.clauses) ? exprRemover.clauses : [];
    for (const clause of clausesAll) {
      if (!(clause instanceof Set)) continue;
      for (const term of Array.from(clause)) {
        if (!clean(term)) continue;
        addLinha(term);
        const itensEstruturais = extrairOrigensEstruturaisDoBloco(term);
        for (const it of itensEstruturais) addLinha(it);
      }
    }
  }

  // Último fallback textual: canonical completo quando ainda não extraiu origem.
  if (!out.size) {
    const canon = clean(exprRemover?.canonical || '');
    if (canon) {
      addLinha(canon);
      const itensEstruturais = extrairOrigensEstruturaisDoBloco(canon);
      for (const it of itensEstruturais) addLinha(it);
    }
  }
...
function detectarConectivoOrigens(firstBlockText, origens) {
  const itens = Array.isArray(origens) ? origens.filter(Boolean) : [];
  if (itens.length <= 1) return 'SINGLE';
  const txt = ((typeof rmAcc === 'function') ? rmAcc(removerDecoradoresVisuaisLocalizador(firstBlockText || '')) : removerDecoradoresVisuaisLocalizador(firstBlockText || ''))
    .toUpperCase()
    .replace(/\s+/g, ' ')
    .trim();
  if (/\sOU\s/.test(txt)) return 'OU';
  if (/\sE\s/.test(txt)) return 'E';
  return 'OU';
}

function extrairOrigemRemoverExpr(td) {
  const tipoCanonico = arguments[1] || '';
  const expr = extrairLocalizadorRemover(td);
  const firstBlockText = extrairPrimeiroBlocoTextoRemover(td);
  const origens = extrairListaOrigensRemover(td, expr, firstBlockText, tipoCanonico);
  return {
    canonical: expr?.canonical || '',
    clauses: Array.isArray(expr?.clauses) ? expr.clauses : [],
    origens,
    origemConectivo: detectarConectivoOrigens(firstBlockText, origens),
    origemTextoBase: clean(firstBlockText || '')
  };
}
...
function extrairTipoControleCriterio(td) { // Extrai pares "Por X: criterio" e devolve expressão (OR).
  ...
  try { if (typeof limparControlesNaoSemanticos === 'function') limparControlesNaoSemanticos(root); } catch (_) {}
  try { removeAlternarUI(root); } catch (_) {}
  try { stripExpandArtifacts(root); } catch (_) {}
  ...
  const hasPeticaoHeader = rawControles.some((c) => /peti[cç][aã]o/i.test(c));
  ...
  const pares = [];
  const normNoAcc = (v) => String(v || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim();
  const isDataTempoBase = (ctrl) => {
    const s = normNoAcc(ctrl);
    return s === 'por data/tempo' || s === 'data/tempo' || s.includes('data/tempo');
  };
  const resolveDataTempoSubtype = (criterio) => {
    const s = normNoAcc(criterio);
    if (!s) return 'Data/Tempo';
    if (s.includes('tempo no localizador') || /\btempo\b/.test(s)) return 'Data/Tempo: Tempo no Localizador';
    if (s.includes('periodic') || s.includes('todos os dias') || /\bdata\b/.test(s)) return 'Data/Tempo: Data ou Periodicamente';
    return 'Data/Tempo';
  };
  const canonicalControle = (ctrl, criterio) => {
    if (!isDataTempoBase(ctrl)) return clean(ctrl);
    return resolveDataTempoSubtype(criterio);
  };
  const extrairDetalhesPeticao = (txt) => {
    const src = clean(String(txt || '')).replace(/\.\.\.\s*$/g, '');
    if (!src) return [];
    const parts = src
      .split(/\s+\bOU\b\s+|\s+\bE\b\s+|[;,|]/i)
      .map((s) => clean(s))
      .filter(Boolean)
      .map((s) => s.replace(/^PETI[CÇ][AÃ]O\s*-\s*/i, ''))
      .map((s) => s.replace(/\s+/g, ' ').trim())
      .filter((s) => s && s !== '...' && !/^por\s+peti[cç][aã]o$/i.test(s));
    return Array.from(new Set(parts));
  };
  const peticaoDetalhes = extrairDetalhesPeticao(body);
  ...
  const parts = pares
    .map((p) => `${canonicalControle(p.controle, p.criterio)}: ${p.criterio}`)
    .map(clean)
    .filter((v) => !!v && v !== '...' && !/:\s*\.\.\.\s*$/i.test(v));
  if (hasPeticaoHeader) {
    const hasPeticaoSpecific = parts.some((p) => /^por\s+peti[cç][aã]o\s*:\s*.+/i.test(p));
    if (!hasPeticaoSpecific && peticaoDetalhes.length) {
      const detalhe = peticaoDetalhes.sort((a, b) => a.localeCompare(b, 'pt-BR')).join(' | ');
      parts.push(`Por Petição: ${detalhe}`);
    }
  }
  // Evita canonical genérico/truncado de "Por Petição ..." sem tipos.
  if (!parts.length) {
    const fullSafe = clean(full).replace(/\.\.\.\s*$/g, '').trim();
    if (fullSafe && !/^\s*por\s+peti[cç][aã]o\s*\/?\s*$/i.test(fullSafe)) {
      return { canonical: fullSafe, clauses: [new Set([fullSafe])], controles, pares: [], header, rawTerms };
    }
  }
